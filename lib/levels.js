function getEnemyPlaneCount(level) {
  if (level <= 2) return 1;
  if (level <= 5) return 2;
  if (level <= 8) return 3;
  if (level === 9) return 4;
...
function getEnemyAccuracy(level) {
  return Math.min(0.92, 0.38 + level * 0.052);
}
...
export const LEVELS = baseLevels.map((level) => ({
  ...level,
  enemyPlaneCount: getEnemyPlaneCount(level.level),
  enemyAccuracy: getEnemyAccuracy(level.level)
}));
